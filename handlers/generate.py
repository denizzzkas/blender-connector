...
def generate_bpy_code(prompt: str, target_mode: str = "new_scene") -> tuple[str, str]:
    ...
    if "cube" in prompt_lower or "box" in prompt_lower:
        ...
        code_lines.extend([
            ...
            "import mathutils",
            "",
            "# Place character prominently in front of camera or at origin / beside scenes",
            "all_objs = [o for o in bpy.context.scene.objects if o.type in ('MESH', 'CURVE', 'SURFACE')]",
            "if all_objs and is_beside:",
            "    max_x = max([o.location.x for o in all_objs])",
            "    offset_x = max_x + 4.0",
            "elif all_objs:",
            "    # If building in existing movie set, place Webbee at center stage",
            "    offset_x = 0.0",
            "else:",
            "    offset_x = 0.0",
            "offset_y = 0.0",
            ...
            "    for c in list(obj.users_collection): c.unlink(obj)",
